...
export default function SchedulePage() {
  ...
  const [newTopic, setNewTopic] = useState<Record<string, { title: string; priority: 'Low' | 'Medium' | 'High'; dueDate: string }>>({});
  ...
  const fetchAll = async () => {
    setLoading(true);
    try {
      const [sRes, mRes] = await Promise.all([fetch('/api/schedule/subjects'), fetch('/api/schedule/modules')]);
  ...
  const addModule = async () => {
    ...
    if (res.ok) {
      const created = await res.json();
      // created is populated (subjectId populated)
  ...
  const addTopic = async (moduleId: string) => {
    ...
    if (res.ok) {
      ...
      setNewTopic((prev) => ({ ...prev, [moduleId]: { title: '', priority: 'Medium', dueDate: '' } }));
    } else {
  ...
  const toggleTopicCompletion = async (moduleId: string, topicId: string, completed: boolean) => {
    ...
    if (res.ok) {
      ...
      console.error('Failed to toggle topic', await res.text());
    }
  };

  const getBadgeColor = (level: string) =>
    level === 'Easy' ? 'bg-purple-100 text-purple-700' : level === 'Medium' ? 'bg-yellow-100 text-yellow-700' : 'bg-red-100 text-red-700';
        ...
        <section className="grid grid-cols-1 lg:grid-cols-2 gap-6">
          <Card className="bg-white/90">
            ...
            <CardContent className="space-y-4">
              ...
              <Label>Color</Label>
              <Input type="color" value={newSubject.color} onChange={(e) => setNewSubject((p) => ({ ...p, color: e.target.value }))} />
          ...
          <Card className="bg-white/90">
            ...
            <CardContent className="space-y-4">
              <Label>Subject</Label>
              <Select value={newModule.subjectId} onValueChange={(v) => setNewModule((p) => ({ ...p, subjectId: v }))}>
                <SelectTrigger>
              ...
              <Select value={newModule.difficulty} onValueChange={(v) => setNewModule((p) => ({ ...p, difficulty: v as any }))}>
                <SelectTrigger>
                  <SelectValue />
                </SelectTrigger>
                <SelectContent>
              ...
              <Label>Hours</Label>
              <Input type="number" min={1} value={newModule.estimatedHours} onChange={(e) => setNewModule((p) => ({ ...p, estimatedHours: e.target.value }))} />
          ...
          {loading ? (
            ...
            subjects.map((s) => {
              const sModules = modules.filter((m) => {
                const sid = typeof m.subjectId === 'string' ? m.subjectId : (m.subjectId as any)?._id;
                return sid === s._id;
              });
                ...
                <Card key={s._id} className="bg-white/90 p-4">
                  <CardHeader className="flex justify-between">
                    <CardTitle className="flex gap-2 items-center">
                      <span style={{ background: s.color }} className="h-4 w-4 rounded-full inline-block" /> {s.name}
                    </CardTitle>
                      ...
                      <div key={m._id} className="p-4 rounded border bg-gray-50">
                        ...
                        <div className="mt-3 space-y-2">
                          <Input
                            ...
                            onChange={(e) => setNewTopic((p) => ({ ...p, [m._id]: { ...(p[m._id] || { priority: 'Medium', dueDate: '' }), title: e.target.value } }))}
                          />
                          <div className="flex gap-2">
                            <Select
                              value={newTopic[m._id]?.priority || 'Medium'}
                              onValueChange={(v) => setNewTopic((p) => ({ ...p, [m._id]: { ...(p[m._id] || { title: '', dueDate: '' }), priority: v as any } }))}
                            >
                            ...
                            <Input
                              ...
                              onChange={(e) => setNewTopic((p) => ({ ...p, [m._id]: { ...(p[m._id] || { title: '', priority: 'Medium' }), dueDate: e.target.value } }))}
                            />
                          </div>
                          <Button onClick={() => addTopic(m._id)} className="bg-blue-600 text-white w-full">
                            Add Topic
                          </Button>
                        </div>
